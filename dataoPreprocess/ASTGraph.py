...
class ASTGraph:
    ...
    def get_source_graph(self):
        if not self.__graph:
            ...
            # 2. get ComputedFrom edge
            computer_edges = []
            operand_edges = []
            for node in self.__ast_nodes:
                if node['type'] in self.__operator['UnaryOperator']:
                    computer_edges.append([node['key'] + 1, 4, node['key'] + 1])  # self edges
                    operand_edges.append([node['key'] + 1, 2, node['key']])       # self and others
                elif node['type'] == '=':
            ...
            # 3. get edges from common memories
            call_function_edges = []
            formal_arg_edges = []
            var_last_use_edges = []
            for key in self.__common_memories:
                ...
                # 4. get CallFunction edge and FormalArgName edge
                if self.__ast_nodes[nodes_key[0]]['type'] == 'FunctionDecl' \
                        or self.__ast_nodes[nodes_key[0]]['type'] == 'CXXMethodDecl':
                    if common_length > 1:
                        for x in range(1, len(nodes_key)):
                            # 4.1  get CallFunction edge via common memories array
                            call_function_edges.append([nodes_key[0], 7, nodes_key[x]])
                            # 4.2  get FormalArgName edge via functions information array and common memories array
            ...
            # 5. get ReturnsTo edge via functions information array
            return_edges = []
